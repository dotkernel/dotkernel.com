...
This applies to PHP 5.x and PHP 7. Floating-point arithmetic doesn't always produce the results you'd expect, especially when casting values to int, because numbers like 0.7 and 0.1 cannot be represented exactly in binary. The result is that `(int)((0.7+0.1)*10)` evaluates to 7 instead of the mathematically expected 8.
