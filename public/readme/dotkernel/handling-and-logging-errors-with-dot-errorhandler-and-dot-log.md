...
This article is a follow-up to "Logging with dot-log in Zend Expressive and DotKernel" and explains how to use `dotkernel/dot-errorhandler` together with `dotkernel/dot-log` or `zendframework/zend-log` to log errors in Zend Expressive applications. It covers how dot-errorhandler was built, how to configure it, and how it was tested.
...
When the dot-errorhandler config provider is invoked, both error handlers' factories are registered, and an alias is added to switch between them. As a fallback, the plain error handler is selected by default, and this can be overwritten through the config file.
...
- dot-errorhandler is meant to be a silent logger for staging and production environments. To test it, development mode should be disabled, otherwise whoops will catch the errors and show them to the developer instead. Any custom error handler can be used as long as it implements the provided `ErrorHandlerInterface`.
...
The `logger` key in the dot-errorhandler config should reflect your logger configuration in `config/autoload/log.global.php`. An out-of-the-box config was provided within the error handler's config directory for using the default logger.
