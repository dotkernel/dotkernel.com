...
The DotKernel project must have a **composer.json** file so that Composer can work. It should look like this:
...
But what if the file does not exist, or Composer is not present? First make sure the Composer autoload path exists, and only load the dependencies if the autoload file was found:
