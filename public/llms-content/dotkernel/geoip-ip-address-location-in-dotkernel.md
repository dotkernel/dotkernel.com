...
A: GeoIP is MaxMind's proprietary technology for IP geolocation data.
DotKernel uses it to get user statistics by country, determining a visitor's country, region, city, postal code, or area code in real time.
...
A: If GeoIPCity.dat exists, it uses the PHP function geoip_record_by_name to get the country code and name.
If neither .dat file exists, it behaves the same as when mod_geoip isn't installed, falling back to externals/geoip/GeoIP.dat.
