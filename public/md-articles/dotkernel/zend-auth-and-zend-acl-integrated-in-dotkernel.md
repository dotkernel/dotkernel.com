...
[Zend_Auth](http://framework.zend.com/manual/en/zend.auth.html) and [Zend_Acl](http://framework.zend.com/manual/en/zend.acl.html) have been integrated into the Dotkernel, starting with version 1.5.0
...
ACL roles (user types) and permissions are configured in the *configs/acl/role.xml* file. In Dotkernel there are 3 roles: *guest*, *user* and *admin*. The resources for ACL are taken from *configs/router.xml* – see *controllers* tag.
...
A: Dot_Acl controls user access (Access Controller Layer), is used for setting and checking a user's permissions, and uses Zend_Acl to check whether a role (user) has access to a resource (controller). Dot_Auth calls the isAllowed method from Dot_Acl to authenticate the user.
