...
On one recent project that used WURFL, response time was an important factor.
Profiling revealed that the greatest chunk of response time (up to a few hundred milliseconds) was taken up by WURFL.
The default filesystem cache turned out to be too slow for a relatively high-traffic application.
...
A: Device data is stored in a large zipped XML file.
On first use, WURFL unzips the file, serializes each device's data, and writes it to cache using an MD5 signature of the user agent as the key.
Because devices are stored as a tree inheriting properties from parent nodes, each lookup requires reading and merging several files.
...
A: Adding a second cache layer on top of WURFL's own cache, using APC and storing arrays of only the specific fields they actually needed in User Cache Entries.
The change was under 10 lines of code.
