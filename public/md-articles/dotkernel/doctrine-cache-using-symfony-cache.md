...
The [dotkernel/dot-cache](https://packagist.org/packages/dotkernel/dot-cache) component is a wrapper that sits on top of [symfony/cache](https://packagist.org/packages/symfony/cache). It currently supports two adapters and can store data in two distinct locations:
...
```
$this->getQueryBuilder()     ->select('admin')     ->from(Admin::class, 'admin')     ->setCacheable(true)     ->getQuery()     ->getResult();
```

Caching is not limited to entities alone. Objects can be cached too. Check the [basic cache usage](https://symfony.com/doc/current/components/cache.html#basic-usage-psr-6) for this purpose.
