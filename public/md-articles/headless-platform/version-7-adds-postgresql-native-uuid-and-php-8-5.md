...
The most important change in v7 is the introduction of support for native UUID. We use the package [ramsey/uuid](https://github.com/ramsey/uuid) to generate the uuid and then store it in the database. In this way we have full control over the UUID version in use. This solution means you don't depend on extensions or a particular version of the database.
...
A: v7 adds support for UUID v7 using the native UUID data type, support for PostgreSQL, support for PHP 8.5, support for a database table prefix, improved database configuration, and replaces the binary data type for id (index) columns in favor of uuid.
