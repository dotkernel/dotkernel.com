...
4. Identify any temporary hack with a special mark, for example:
   ...
   ```
5. Each function must do a single task.
If it logs in the user and records the login in a stats table, create a separate function for the "record the login" part - maybe even a distinct class for stats.
